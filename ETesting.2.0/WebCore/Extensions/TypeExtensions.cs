	...
	public static class TypeExtensions
	{
		...
		public static void MergeHtmlAttributes(this TagBuilder tb, IDictionary<string, object> htmlAttributes)
		{
			...
			foreach (KeyValuePair<string, object> current in htmlAttributes)
			{
				...
				{
					tb.Attributes.Add(current.Key, current.Value?.ToString() ?? string.Empty);
				}
				else
				{
					if (current.Key.ToLower() == "class")
		...
		public static string GetEnumDescription(this Enum enumerationValue)
		{
			...
			if (member.Length > 0)
			{
				...
				var customAttributes2 = member[0].GetCustomAttributes(typeof(DisplayAttribute), false);
		...
		public static IEnumerable<SelectListItem> SelectListItemsFromEnumMetadata(this ModelMetadata metadata)
		{
		    ...
		    return Enum.GetValues(type).OfType<Enum>().Select(current => new SelectListItem
		    {
		        Value = Enum.Parse(type, current.ToString()).ToString(), Text = current.GetEnumDescription(), Selected = current.Equals(metadata.Model)
		    }).ToList();
		}
